...
def main(
    ...
    if mix_ratio > 0.0:
        ...
        mixed_ds = create_mixed_supervision_dataset(
            weak_labeled_ds=weak_ds,      # Dataset with weak model predictions
            ground_truth_ds=train2_ds,     # Dataset with ground truth labels
            mix_ratio=mix_ratio,
            mix_strategy=mix_strategy,
            seed=seed
        )
        ...
        # Pure weak supervision (original behavior)
        transfer_train_ds = weak_ds
    ...
    for tloss in transfer_losses:
        ...
        transfer_test_results, _ = train_model(
            strong_model_config,
            transfer_train_ds,  # Use mixed dataset (or weak_ds if mix_ratio=0)
            test_ds,
            loss_type=tloss,
            label="weak2strong",
    ...
    res_dict = {
        ...
        "mix_ratio": mix_ratio,
        "mix_strategy": mix_strategy if mix_ratio > 0 else "none",
    }
